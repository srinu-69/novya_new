...
const LearningReports = () => {
  ...
  // NEW: Get and update streak
  const [streakData, setStreakData] = useState({ streak: 0, lastVisitDate: null });
  ...
  const getTotalStatistics = () => {
    ...
    const totalQuizzes = Math.max(
      quizHistory.length, 
      quizResults?.totalQuizzes || 0
    );
    const totalMockTests = Math.max(
      mockHistory.length,
      mockTestResults?.totalTests || 0
    );
   
    // Total questions answered - calculate from history if available
    const totalQuizQuestions = quizHistory.length > 0 
      ? quizHistory.reduce((total, quiz) => total + (quiz.questions || quiz.questionsCount || 0), 0)
      : (quizResults?.totalQuestions || 0);
    const totalMockQuestions = mockHistory.length > 0
      ? mockHistory.reduce((total, mock) => total + (mock.questions || mock.questionsCount || 0), 0)
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      setError(null);
      try {
        // Try to fetch daily summary from API first, but also get total statistics
        let totalStatsFromAPI = null;
        try {
          // Fetch total statistics from quiz statistics endpoint
          try {
            const statsResponse = await djangoAPI.get(API_CONFIG.DJANGO.QUIZZES.STATISTICS);
            if (statsResponse) {
              // Parse the response - API returns nested structure with 'quiz' and 'mock_test' keys
              const quizCount = statsResponse.quiz?.total_attempts || statsResponse.total_quiz_attempts || 0;
              const mockCount = statsResponse.mock_test?.total_attempts || statsResponse.total_mock_test_attempts || 0;
              
              // Get total activities from lessonHistory
              const totalActivitiesCompleted = lessonHistory.filter(activity =>
                activity.completed === true &&
                activity.activityType !== 'feedback' &&
                activity.activityType !== 'typing_practice' &&
              ...
              totalStatsFromAPI = {
                ...
                totalMockTests: mockCount,
                totalActivitiesCompleted: totalActivitiesCompleted,
                totalStudyTime: getTotalScreenTime(), // Always use current screenTime from screenTime context
                rewardPoints: rewardPoints || 0
              };
              console.log('✅ Total statistics fetched from API:', totalStatsFromAPI);
            }
          } catch (statsError) {
            console.warn('Could not fetch total statistics from API, will use local calculation:', statsError);
          }

          // Try to fetch daily summary
          try {
            const apiResponse = await djangoAPI.get(
              `${API_CONFIG.DJANGO.AUTH.DAILY_SUMMARY}?date=${currentDate}`
            );
            
            if (apiResponse) {
              // Get total statistics - use API totals if available, otherwise calculate locally
              ...
              const transformedDailyReport = {
                ...
                activities_completed: (apiResponse.quizzes_completed || 0) + 
                                     ...
                                     (apiResponse.classroom_activities || 0),
                productivity_score: Math.min(10, Math.floor(
                  ((apiResponse.total_study_time_minutes || 0) / 60) * 2 + 
                  ((apiResponse.quizzes_completed || 0) + (apiResponse.mock_tests_completed || 0)) * 1.5
                )),
                activities: apiResponse.activity_summary?.activities || [],
                total_statistics: finalTotalStats, // Use calculated total stats, not just today's
                coins_earned: apiResponse.coins_earned || 0
              };
            ...
            console.warn('Could not fetch daily summary from API, will use local calculation:', dailyError);
          }
        } catch (apiError) {
  ...
  // UPDATED: Total Statistics Component with formatted time, streaks, screentime, and reward points
  const TotalStatistics = ({ stats }) => {
    const trophyTitle = getTrophyTitle(streakData.streak);
    ...
    return (
      <div className="total-statistics">
        ...
        <div className="stats-grid total-stats-grid">
          ...
          {/* NEW: Streaks Display */}
          <div className="total-stat-item">
            <span className="total-stat-value">🔥 {streakData.streak}</span>
            <span className="total-stat-label">
              Day Streak {trophyTitle && `• ${trophyTitle}`}
            </span>
          </div>
