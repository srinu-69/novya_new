    ...
    try:
        ...
        if user and hasattr(user, 'username') and user.username:
            try:
                ...
                try:
                    return StudentRegistration.objects.get(student_email=user.email)
                except StudentRegistration.DoesNotExist:
                    return None
        else:
            return None
...
def submit_mock_test_logic(request, validated_data, student_reg):
    ...
    # Handle both 'quiz_questions' and 'test_questions' field names
    test_questions = validated_data.get('test_questions', validated_data.get('quiz_questions', []))
    ...
    # Handle both 'quiz_type' and 'test_type' field names from serializer
    quiz_type = validated_data.get('quiz_type', validated_data.get('test_type', 'mock_test'))
    
    # Debug logging
    print(f"📊 Creating mock test attempt with data:")
    print(f"   subject: {validated_data.get('subject', 'NOT SET')}")
    print(f"   chapter: {validated_data.get('chapter', 'NOT SET')}")
    print(f"   topic: {validated_data.get('topic', 'NOT SET')}")
    print(f"   subtopic: {validated_data.get('subtopic', 'NOT SET')}")
    print(f"   class_name: {class_name}")
    print(f"   total_questions: {validated_data.get('total_questions', 'NOT SET')}")
    print(f"   correct_answers: {validated_data.get('correct_answers', 'NOT SET')}")
    print(f"   answers_json: {answers_json[:100] if answers_json else 'EMPTY'}...")
    
    attempt = MockTestAttempt.objects.create(
        test_id=dummy_mock_test,
        student_id=student_reg,
        score=validated_data.get('score', 0),
        answers_json=answers_json if answers_json else None,
        quiz_type=quiz_type,
        subject=validated_data.get('subject', '') or None,
        chapter=validated_data.get('chapter', '') or None,
        topic=validated_data.get('topic', '') or None,
        subtopic=validated_data.get('subtopic', '') or None,
        class_name=class_name or None,
        difficulty_level=validated_data.get('difficulty_level', '') or None,
        language=validated_data.get('language', '') or None,
        total_questions=validated_data.get('total_questions', 0) or 0,
        correct_answers=validated_data.get('correct_answers', 0) or 0,
        wrong_answers=validated_data.get('wrong_answers', 0) or 0,
        unanswered_questions=validated_data.get('unanswered_questions', 0) or 0,
        time_taken_seconds=validated_data.get('time_taken_seconds', 0) or 0,
        completion_percentage=validated_data.get('score', 0) or 0,
        mock_test_data_json=test_data_json if test_data_json else None
    )
    
    print(f"✅ Mock test attempt created with ID: {attempt.attempt_id}")
    print(f"   Stored subject: {attempt.subject}")
...
def submit_mock_test_attempt(request):
    ...
    # Debug logging
    print(f"📥 Received mock test submission request:")
    print(f"   Data keys: {list(request.data.keys())}")
    ...
    if serializer.is_valid():
        ...
        # Debug logging
        print(f"📊 Creating mock test attempt via submit_mock_test_attempt:")
        print(f"   subject: {validated_data.get('subject', 'NOT SET')}")
        print(f"   chapter: {validated_data.get('chapter', 'NOT SET')}")
        print(f"   topic: {validated_data.get('topic', 'NOT SET')}")
        print(f"   subtopic: {validated_data.get('subtopic', 'NOT SET')}")
        ...
        # Create mock test attempt with ALL fields (same as submit_mock_test_logic)
        attempt = MockTestAttempt.objects.create(
            test_id=dummy_mock_test,
            student_id=student_reg,
            score=validated_data.get('score', 0),
...
def get_student_performance(request):
    ...
    for attempt in quiz_attempts:
        ...
        subject_performance[subject]['total_questions'] += attempt.total_questions or 0
        subject_performance[subject]['correct_answers'] += attempt.correct_answers or 0
        subject_performance[subject]['total_score'] += attempt.score or 0
    
    # Process mock test attempts
    for attempt in mock_test_attempts:
        subject = attempt.subject or 'Mock Test'  # Use subject from attempt, or default to "Mock Test"
        if subject not in subject_performance:
            subject_performance[subject] = {
                'total_attempts': 0,
                'total_questions': 0,
        ...
        subject_performance[subject]['total_attempts'] += 1
        # Use actual values from model, with fallback to defaults
        total_questions = attempt.total_questions or 50  # Default to 50 for mock tests if not set
        correct_answers = attempt.correct_answers or int(attempt.score or 0)  # Use correct_answers if available, otherwise use score
        subject_performance[subject]['total_questions'] += total_questions
        subject_performance[subject]['correct_answers'] += correct_answers
        subject_performance[subject]['total_score'] += attempt.score or 0
    ...
    for attempt in quiz_attempts:
        ...
        class_performance[class_name]['total_questions'] += attempt.total_questions or 0
        class_performance[class_name]['correct_answers'] += attempt.correct_answers or 0
    ...
    for attempt in quiz_attempts:
        ...
        difficulty_performance[difficulty]['total_questions'] += attempt.total_questions or 0
        difficulty_performance[difficulty]['correct_answers'] += attempt.correct_answers or 0
