...
function MockTest() {
  ...
  // Start time tracking when mock test begins
  useEffect(() => {
    if (selectedChapter && quiz.length > 0 && !isFinished && !showInstructions) {
      console.log("Starting mock test time tracking...");
      const activityId = startActivityTracking('mock_test', {
        testId: `mock-${selectedChapter}-${Date.now()}`,
        subject: selectedSubject,
        class: selectedClass,
        chapter: selectedChapter,
      ...
      console.log("Mock test activity started with ID:", activityId);
    }
  }, [selectedChapter, quiz.length, isFinished, showInstructions]);

  // Stop time tracking when component unmounts or test finishes
  useEffect(() => {
    return () => {
      if (currentActivityId && !isFinished) {
        console.log("Stopping mock test time tracking (component unmount)...");
  ...
  }, [currentActivityId, isFinished, score, quiz.length, currentQ]);
  ...
  const finishQuiz = async () => {
    ...
    // Stop time tracking when test finishes
    if (currentActivityId) {
      console.log("Stopping mock test time tracking (test finished)...");
      stopActivityTracking(currentActivityId, {
        score: score,
        totalQuestions: quiz.length,
        completed: true,
        ...
        passed: passed,
        timeSpent: (20 * 60 - timeLeft) // actual time spent in seconds
      });
      setCurrentActivityId(null);
    }
  ...
  const retryQuiz = () => {
    ...
    if (currentActivityId) {
      console.log("Stopping mock test time tracking (retry)...");
      stopActivityTracking(currentActivityId, {
        score: score,
        totalQuestions: quiz.length,
        completed: false,
  ...
  const nextLevel = () => {
    ...
    if (currentActivityId) {
      console.log("Stopping mock test time tracking (next level)...");
      stopActivityTracking(currentActivityId, {
        score: score,
        totalQuestions: quiz.length,
        completed: true,
        levelCompleted: 'normal'
      });
      setCurrentActivityId(null);
    }
  ...
  const backToChapters = () => {
    ...
    if (currentActivityId && !isFinished) {
      console.log("Stopping mock test time tracking (back to chapters)...");
      stopActivityTracking(currentActivityId, {
        score: score,
        totalQuestions: quiz.length,
        completed: false,
  ...
  const backToSubjects = () => {
    ...
    if (currentActivityId) {
      stopActivityTracking(currentActivityId, {
        completed: false,
        reason: 'navigated_away_from_subject'
      });
      setCurrentActivityId(null);
    }
