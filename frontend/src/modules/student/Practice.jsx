...
//   const getWheelData = () => [
//     { option: t('spinWheel.reward.10Points'), style: { backgroundColor: '#FF6B6B', textColor: 'white' } },
//     { option: t('spinWheel.reward.20Points'), style: { backgroundColor: '#4ECDC4', textColor: 'white' } },
//     { option: t('spinWheel.reward.05Points'), style: { backgroundColor: '#45B7D1', textColor: 'white' } },
//     { option: t('spinWheel.reward.bonusSpin'), style: { backgroundColor: '#96CEB4', textColor: 'white' } },
//     { option: t('spinWheel.reward.15Points'), style: { backgroundColor: '#FFEAA7', textColor: 'black' } },
//     { option: t('spinWheel.reward.betterLuck'), style: { backgroundColor: '#DDA0DD', textColor: 'white' } },
//   ];

//   // Internationalized rewards mapping - CORRECTED FOR PROPER ALIGNMENT
//   const getRewardForSegment = (segmentIndex) => {
//     const rewards = [
//       { id: 1, name: t('spinWheel.reward.10Points'), color: '#FF6B6B', value: 10 },      // Index 0 - 10 Points
//       { id: 2, name: t('spinWheel.reward.20Points'), color: '#4ECDC4', value: 20 },      // Index 1 - 20 Points  
//       { id: 3, name: t('spinWheel.reward.05Points'), color: '#45B7D1', value: 5 },       // Index 2 - 05 Points
//       { id: 4, name: t('spinWheel.reward.bonusSpin'), color: '#96CEB4', value: 1 },      // Index 3 - Bonus Spin
//       { id: 5, name: t('spinWheel.reward.15Points'), color: '#FFEAA7', value: 15 },      // Index 4 - 15 Points
//       { id: 6, name: t('spinWheel.reward.betterLuck'), color: '#DDA0DD', value: 0 },     // Index 5 - Better Luck
//     ];
...
//           }}>
//             {[
//               { name: t("Eng_lish"), icon: '📖', color: '#F59E0B', tests: 420 },
//               { name: t("Tel_ugu"), icon: '🔤', color: '#EC4899', tests: 380 },
//               { name: t("Mathe_matics"), icon: '🔢', color: '#3B82F6', tests: 450 },
//               { name: t("Soc_ial"), icon: '🌏', color: '#8B5CF6', tests: 340 },
//               { name: t("Sci_ence"), icon: '🔬', color: '#10B981', tests: 390 },
//               { name: t("Comp_uters"), icon: '💻', color: '#6366F1', tests: 310 }
//             ].map((subject, index) => (
//               <div
//                 key={index}
...
// Spin Wheel Component with full internationalization
const SpinWheel = ({ onRewardWon, onSpinActivityTracked }) => { // UPDATED: Added onSpinActivityTracked prop
  const { t, i18n } = useTranslation();
  ...
  const trackSpinWheelActivity = (rewardWon, pointsAwarded) => {
    try {
      const activityData = {
        ...
        subject: 'Rewards',
        chapter: 'Daily Bonus',
        duration: 1, // Quick activity
        rewardPoints: pointsAwarded,
        rewardName: rewardWon.name,
        rewardValue: rewardWon.value,
        spinsRemaining: spinsLeft - 1,
        completed: true
      };

      // Track the activity
      trackLearningActivity(activityData);
      console.log('🎰 Spin wheel activity tracked in learning history');

      // Call the callback if provided
      if (onSpinActivityTracked) {
        onSpinActivityTracked(activityData);
      }
    } catch (error) {
      console.error('Error tracking spin wheel activity:', error);
    }
  };
  ...
  const handleStopSpinning = () => {
    ...
    // NEW: Track spin wheel activity
    trackSpinWheelActivity(wonReward, wonReward.value);
...
const Practice = () => {
  ...
  // NEW: Function to handle spin wheel activity tracking
  const handleSpinActivityTracked = (activityData) => {
    console.log('🎰 Spin wheel activity tracked:', activityData);
          ...
          >
            ...
            >
              <SpinWheel 
                ...
                onSpinActivityTracked={handleSpinActivityTracked} // UPDATED: Added callback
              />
            </div>
          </div>
        </div>
